...
print("=== Processing Demolishor Vehicle Textures ===")

# 1. Albedo / Diffuse
diffuse_path = SRC_DIR / "vh_demolishor_normspec_cust_e_mat_inst.jpeg"
print(f"Loading Vehicle Diffuse from {diffuse_path.name}...")
diffuse_img = Image.open(diffuse_path).convert("RGBA")
diffuse_1024 = diffuse_img.resize((1024, 1024), Image.Resampling.LANCZOS)
diffuse_out = OUT_DIR / "cha_demolishor_vh_a.png"
diffuse_1024.save(diffuse_out, format="PNG")
print(f"[✓] Saved {diffuse_out.name} (1024x1024)")

# 2. Normal Map (Invert Green channel for Unity OpenGL normal format)
normal_path = SRC_DIR / "vh_demolishor_n.jpeg"
print(f"Loading Vehicle Normal from {normal_path.name}...")
...
normal_out = OUT_DIR / "cha_demolishor_vh_NM.png"
...
# R: Roughness (~65)
# G: AO (~180)
# B: Emissive (from vh_demolishor_e.jpeg)
emissive_path = SRC_DIR / "vh_demolishor_e.jpeg"
...
raoe_out = OUT_DIR / "cha_demolishor_vh_tform_misc_RAOE.png"
