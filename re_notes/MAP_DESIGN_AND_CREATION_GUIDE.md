...
### 维度 5：战斗流与通关规则控制 (`isLeisure` 开关)
*支持休闲通关模式与原版高难挑战模式的自由切换。*

* **配置方式**：在 `QuestSummary` 与 `availableQuests` 中配置布尔值 `"isLeisure"`：
  - `true`（休闲模式 / 默认）：
    - **战前退出**：玩家在战前准备界面（PreFightScreen）点击返回可直接通关该节点（快速通关/剧情体验）。
    - **全员锁血**：战败后依然判定通过，全员生命值锁定为 100%（`hp: 1.0`），无死亡惩罚。
  - `false`（原版硬核规则）：
    - **拦截偷跑**：在战前准备界面点击返回**无法绕过战斗**；底层重定向拦截前进请求并将玩家锚定在遭遇战格子上，必须战胜才能前进。
    - **真实血量与 KO**：战斗结束由 `hook_157` (`Fight.OnFightEnded`) 判定输赢并提取剩余血量；战败后英雄进入 KO 状态（`hp: 0.00`），棋盘与选人界面显示红底 **KO** 标记；全员阵亡触发关卡失败结算，只有战胜才解锁并允许继续前进。
* **现有关卡规划**：
  - 飞天虎组合金刚挑战 (`1.1.3`)：`isLeisure: false`（硬核 Boss 战）
  - 盖世擎天柱极限单挑 (`1.1.4`)：`isLeisure: false`（硬核 1v1 战）
  - 其他常规关卡（`1.1.1`, `1.1.2`, `1.1.5` 等）：`isLeisure: true`
